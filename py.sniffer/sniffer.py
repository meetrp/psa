...
ports = dict()
with open("etc/known.ports") as ports_file:
...
ethertypes = dict()
with open("etc/ether.types") as types_file:
...
def get_default_route(net_iface_dev):
    with open(DEFAULT_PROC_ROUTE_PATH) as fh:
        for line in fh:
            ...
            if (module_exists("cache") == True):
                g_cache_server.set([ip], "Default-Route")
...
def local_ip():
    ...
    if (module_exists("cache") == True):
        g_cache_server.set([ip], "Local-IP")
...
def is_acceptable_dest(dest_addr, dest_port):
    if dest_addr in g_disallowed_ip:
        return False
    elif int(dest_port) in g_disallowed_port:
        return False
    else:
        return True
...
def main():
    ...
    if not path.isfile(get_sys_path(net_iface_dev)):
        ...
        if (module_exists("cache") == True):
            g_cache_server = Cache()  # Populate the static values
